...
// ---------------------------------------------------------------------------
// PyCapsule helpers for RecordBatch exchange with external .so modules
// ---------------------------------------------------------------------------

/// Serializes a RecordBatch to IPC bytes for passing to external .so decode methods.
///
/// # Errors
///
/// Returns a `PyErr` if creating the IPC writer, writing the batch, or finishing fails.
#[cfg(feature = "python")]
pub fn serialize_record_batch_to_ipc_bytes(
    py: pyo3::Python<'_>,
    ...
    let mut buffer = Vec::new();
    {
        let mut writer = arrow::ipc::writer::FileWriter::try_new(&mut buffer, &batch.schema())
            .map_err(|e| {
                pyo3::exceptions::PyValueError::new_err(format!("Failed to create IPC writer: {e}"))
            })?;
        writer.write(&batch).map_err(|e| {
            pyo3::exceptions::PyValueError::new_err(format!("Failed to write IPC batch: {e}"))
        })?;
        writer.finish().map_err(|e| {
            pyo3::exceptions::PyValueError::new_err(format!("Failed to finish IPC writer: {e}"))
        })?;
    }
    Ok(PyBytes::new(py, &buffer).into_any().unbind())
}

/// Extracts a RecordBatch from IPC bytes produced by `encode_record_batch_py`.
///
/// # Errors
///
/// Returns an error if the input is not bytes, the IPC reader fails, or no batch is present.
#[cfg(feature = "python")]
pub fn extract_record_batch_from_ipc_bytes(
    py_batch: pyo3::Bound<'_, pyo3::PyAny>,
) -> anyhow::Result<arrow::record_batch::RecordBatch> {
    use pyo3::types::PyBytes;

    let bytes: &pyo3::Bound<'_, PyBytes> = py_batch
        .cast::<PyBytes>()
        .map_err(|_| anyhow::anyhow!("Expected bytes from encode_record_batch_py"))?;

    let data = bytes.as_bytes();
    let cursor = std::io::Cursor::new(data);
    let reader = arrow::ipc::reader::FileReader::try_new(cursor, None)
        .map_err(|e| anyhow::anyhow!("Failed to create IPC reader: {e}"))?;

    // Read the first (and only) batch
    let batches: Result<Vec<_>, _> = reader.collect();
    let batches = batches.map_err(|e| anyhow::anyhow!("Failed to read IPC batches: {e}"))?;

    batches
        .into_iter()
        .next()
        .ok_or_else(|| anyhow::anyhow!("No batches in IPC data"))
}
...
impl Serialize for CustomData {
    ...
    {
        use serde::ser::Error;
        let json = self.data.to_json().map_err(Error::custom)?;
        let mut value: serde_json::Value = serde_json::from_str(&json).map_err(Error::custom)?;
        if let Some(obj) = value.as_object_mut() {
            obj.insert(
                "type".to_string(),
                serde_json::Value::String(self.data.type_name().to_string()),
            );
            ...
            obj.insert(
                "data_type".to_string(),
                serde_json::Value::Object(data_type_obj),
            );
        }
