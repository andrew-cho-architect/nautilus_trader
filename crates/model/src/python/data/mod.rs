...
#[cfg(feature = "python")]
use nautilus_core::python::params::{params_to_pydict, pydict_to_params};
#[cfg(feature = "python")]
...
use crate::data::{
    ...
    custom::{extract_record_batch_from_ipc_bytes, serialize_record_batch_to_ipc_bytes},
...
fn py_decode_record_batch_to_custom_data(
    ...
    pyo3::Python::attach(|py| {
        let ipc_bytes = serialize_record_batch_to_ipc_bytes(py, batch)
            .map_err(|e| anyhow::anyhow!("Failed to serialize batch to IPC: {e}"))?;
        ...
        let py_list = data_class
            .bind(py)
            .call_method1("decode_record_batch_py", (metadata_py, ipc_bytes))
...
pub fn register_custom_data_class(data_class: &Bound<'_, PyAny>) -> PyResult<()> {
    ...
    if !data_class.hasattr("decode_record_batch_py")? {
        return Err(pyo3::exceptions::PyTypeError::new_err(
            "Custom data class must have decode_record_batch_py(metadata, ipc_bytes) class method",
        ));
    }
