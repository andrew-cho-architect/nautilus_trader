...
fn encode_field_expr(field_name: &syn::Ident, ty: &Type) -> Option<TokenStream> {
    ...
    match (outer.as_str(), inner.as_str()) {
        ...
        _ if outer == inner => match outer.as_str() {
            ...
            "u64" | "u32" | "i64" => Some(quote! { builder.append_value(item.#name); }),
...
fn decode_field_rhs(
    ...
    match (outer.as_str(), inner.as_str()) {
        ...
        _ if outer == inner => match outer.as_str() {
            ...
            "f64" | "f32" | "bool" | "u64" | "u32" | "i64" => Some(quote! { #col.value(i) }),
...
fn gen_pymethods_impl(ctx: &ExpansionContext<'_>) -> TokenStream {
    ...
    let getters: Vec<TokenStream> = field_list
        ...
        .collect();
    let greeks_from_delta: TokenStream = if name_str == "GreeksData" {
        quote! {
            /// Construct from delta only (multiplier defaults to 1.0). Class method for Python.
            #[classmethod]
            #[pyo3(signature = (instrument_id, delta, ts_event=0))]
            fn from_delta(
                ...
                let ts = nautilus_core::UnixNanos::new(ts_event as u64);
                Ok(Self::from_delta_rust(instrument_id, delta, 1.0, ts))
            }
        }
    } else {
        quote! {}
        ...
        impl #generics #name #generics {
            ...
            fn from_json(
                ...
                let json_module = py.import("json")
                    .map_err(|e| pyo3::exceptions::PyValueError::new_err(format!("import json failed: {e}")))?;
                let json_str: String = json_module
                    .call_method1("dumps", (data,))
                    .map_err(|e| pyo3::exceptions::PyValueError::new_err(format!("json.dumps failed: {e}")))?
                    .extract()?;
                let value: serde_json::Value = serde_json::from_str(&json_str)
                    .map_err(|e| pyo3::exceptions::PyValueError::new_err(format!("serde_json::from_str failed: {e}")))?;
                let arc = <#name as nautilus_model::data::CustomDataTrait>::from_json(value)
                    .map_err(|e| pyo3::exceptions::PyValueError::new_err(e.to_string()))?;
                let inner = arc.as_any().downcast_ref::<#name>()
                    .ok_or_else(|| pyo3::exceptions::PyValueError::new_err("from_json downcast failed"))?;
                Ok(pyo3::Py::new(py, inner.clone())?.into_any())
            }

            /// Decodes a RecordBatch from IPC bytes into a list of instances.
            /// Class method: call via MarketTickData.decode_record_batch_py(metadata, ipc_bytes).
            #[pyo3(signature = (metadata, ipc_bytes))]
            #[classmethod]
            fn decode_record_batch_py(
                _cls: pyo3::Bound<'_, pyo3::types::PyType>,
                py: pyo3::Python<'_>,
                metadata: std::collections::HashMap<String, String>,
                ipc_bytes: &pyo3::Bound<'_, pyo3::types::PyBytes>,
            ) -> pyo3::PyResult<pyo3::Py<pyo3::PyAny>> {
                let batch = nautilus_model::data::custom::extract_record_batch_from_ipc_bytes(
                    ipc_bytes.clone().into_any(),
                ).map_err(|e| pyo3::exceptions::PyValueError::new_err(e.to_string()))?;
            ...
            /// Encodes a batch of items to an Arrow RecordBatch. Returns IPC-serialized bytes
            /// that can be safely passed across .so boundaries.
            fn encode_record_batch_py(
                ...
                nautilus_model::data::custom::serialize_record_batch_to_ipc_bytes(py, batch)
            }
