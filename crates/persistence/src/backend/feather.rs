...
impl FeatherWriter {
    ...
    fn regen_writer_path(
        ...
        let mut p = Path::from(self.base_path.clone());
        ...
        if type_str.starts_with("data/custom/") {
            ...
            p = p.child("data").child("custom").child(type_name.to_string());
            if let Some(ref id) = instrument_id {
                let safe = safe_directory_identifier(id);
                if !safe.is_empty() {
                    for segment in safe.split('/') {
                        p = p.child(segment.to_string());
                    }
                }
            }
            let file_stem = instrument_id.as_deref().unwrap_or(type_name);
            p = p.child(format!("{file_stem}_{timestamp}.feather"));
        } else if let Some(ref instrument_id) = instrument_id {
            p = p.child(type_str.clone());
            p = p.child(format!("{instrument_id}_{timestamp}.feather"));
        } else {
            p = p.child(format!("{type_str}_{timestamp}.feather"));
        }
